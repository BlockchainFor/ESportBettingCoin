...
*Setup instructions for a gitian build of ESportBettingCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ESportBettingCoin
...
to esportbettingcoin-crypto.com.
...
- [Building ESportBettingCoin](#building-esportbettingcoin)
...
Clone the git repositories for esportbettingcoin and gitian and then checkout the esportbettingcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the esportbettingcoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building ESportBettingCoin
----------------

To build ESportBettingCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the esportbettingcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/esportbettingcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/esportbettingcoin-crypto/esportbettingcoin
...
For example:
```bash
URL=https://github.com/crowning-/esportbettingcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit esportbettingcoin=${COMMIT} --url esportbettingcoin=${URL} ../ESportBettingCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit esportbettingcoin=${COMMIT} --url esportbettingcoin=${URL} ../ESportBettingCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit esportbettingcoin=${COMMIT} --url esportbettingcoin=${URL} ../ESportBettingCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/esportbettingcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/esportbettingcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/esportbettingcoin-build.assert
```
...
[esportbettingcoin/gitian.sigs](https://github.com/esportbettingcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
