...
#include "config/esportbettingcoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("esportbettingcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "esportbettingcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "esportbettingcoin.pid"));
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ESBC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ESBC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, esportbettingcoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ESBC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all e-Sport Betting Coin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizeesportbettingcoinamount=<n>", strprintf(_("Keep N ESBC anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
	   FormatParagraph(_("Copyright (C) 2018 The e-Sport Betting Coin Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("esportbettingcoin-loadblk");
...
/** Sanity checks
 *  Ensure that e-Sport Betting Coin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. e-Sport Betting Coin Core is shutting down."));
...
#endif
    // Make sure only a single e-Sport Betting Coin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. e-Sport Betting Coin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("e-Sport Betting Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of e-Cport Betting Coin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart e-Sport Betting Coin Core to complete") << "\n";
    ...
    nAnonymizeEsportbettingcoinAmount = GetArg("-anonymizeesportbettingcoinamount", 0);
    if (nAnonymizeEsportbettingcoinAmount > 999999) nAnonymizeEsportbettingcoinAmount = 999999;
    if (nAnonymizeEsportbettingcoinAmount < 2) nAnonymizeEsportbettingcoinAmount = 2;
    ...
    LogPrintf("Anonymize e-Sport Betting coin Amount %d\n", nAnonymizeEsportbettingcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1ESBC+1000 == (.1ESBC+100)*10
       10ESBC+10000 == (1ESBC+1000)*10
    */
