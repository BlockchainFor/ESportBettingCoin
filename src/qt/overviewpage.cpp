...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ESBC")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeEsportbettingcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEsportbettingcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeEsportbettingcoinAmount = strAnonymizeEsportbettingcoinAmount.remove(strAnonymizeEsportbettingcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEsportbettingcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeEsportbettingcoinAmount * COIN) nMaxToAnonymize = nAnonymizeEsportbettingcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeEsportbettingcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeEsportbettingcoinAmount));
        strAnonymizeEsportbettingcoinAmount = strAnonymizeEsportbettingcoinAmount.remove(strAnonymizeEsportbettingcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEsportbettingcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeEsportbettingcoinAmount == 0) {
